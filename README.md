...
События записываются только при включённом профайлере и активной трассе: вне трассы collector никто
не прочитает и не очистит. Список событий ограничен 5 000 записями (`new MultiTenantProfilerCollector(maxItems: ...)`),
после предела растёт только счётчик, а в `summary` появляются `truncated` и `dropped`. Подробнее — в README
`phpsoftbox/profiler`, раздел «Память в долгих процессах».
