...
final readonly class TenantResolveMiddleware implements MiddlewareInterface
{
    ...
    public function __construct(
        ...
        private TenantRequestResolverInterface $resolver = new HostTenantRequestResolver(),
    ) {
    }
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        try {
            ...
            $tenant = $this->profileResolveRequest($request, $tags);
    ...
    private function profileResolveRequest(ServerRequestInterface $request, array $tags): ?TenantDefinition
    {
        $tags['resolver'] = get_class($this->resolver);
        $start            = hrtime(true);
        $span             = $this->profiler?->currentTrace() !== null
            ? $this->profiler->start('tenant.request.resolve', $tags, 'multi_tenant')
            : null;

        try {
            $tenant = $this->resolver->resolve($request, $this->tenants);
            $this->recordProfilerEvent('tenant.request.resolve', $tags + [
