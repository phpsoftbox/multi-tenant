...
final class TenantCommandProvider implements CommandProviderInterface
{
    public function register(CommandRegistryInterface $registry): void
    {
        ...
        $registry->register(Command::define(
            ...
            handler: TenantAuthRoleSyncHandler::class,
        ));

        $registry->register(Command::define(
            name: 'tenant:auth:role:assign',
            description: 'Назначает роль пользователю в core и/или tenant runtime',
            signature: [
                ...
                new OptionDefinition(
                    name: 'scope',
                    short: 's',
                    description: 'Область назначения: all|core|tenant',
                    required: false,
                    default: 'tenant',
                    type: 'string',
                ),
                new OptionDefinition(
                    name: 'tenant',
                    short: 't',
                    description: 'Tenant ID или all (обязательно для tenant scope)',
                    required: false,
                    default: null,
                    type: 'string',
                ),
            ],
            handler: TenantAuthRoleAssignHandler::class,
        ));

        $registry->register(Command::define(
            name: 'tenant:auth:role:discharge',
            description: 'Снимает роль с пользователя в core и/или tenant runtime',
            signature: [
                new ArgumentDefinition('user', 'ID пользователя', required: true),
                new ArgumentDefinition('role', 'Имя роли', required: true),
                new OptionDefinition(
                    name: 'scope',
                    short: 's',
                    description: 'Область снятия: all|core|tenant',
                    required: false,
                    default: 'tenant',
                    type: 'string',
                ),
            ...
            handler: TenantAuthRoleDischargeHandler::class,
        ));
