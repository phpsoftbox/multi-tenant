...
final readonly class TenantAuthRoleDischargeHandler extends AbstractTenantAuthRoleHandler
{
    public function run(RunnerInterface $runner): int|Response
    {
        ...
        $ok = $this->runForScope(
            ...
            operation: static function (ScopedAuthRoleOperationsInterface $operations, string $label) use ($runner, $user, $role): void {
                $operations->dischargeRole($user, $role);
                $runner->io()->writeln('[' . $label . '] Роль ' . $role . ' снята с пользователя ' . $user . '.', 'success');
            },
