...
final readonly class TenantProvisionRunHandler implements HandlerInterface
{
    public function __construct(
        private TenantSelector $selector,
        private TenantProvisionRunnerInterface $runnerService,
    ) {
    }
    ...
    public function run(RunnerInterface $runner): int|Response
    {
        ...
        if ($tenants === []) {
            $runner->io()->writeln('Не найдено tenant для run provisioning.', 'warning');
        ...
        foreach ($tenants as $item) {
            ...
            try {
                $this->runnerService->run($payload);
            } catch (Throwable $exception) {
                $runner->io()->writeln(
                    '[tenant:' . $item->id . '] ошибка provisioning: ' . $exception->getMessage(),
                    'error',
                );
            ...
            $runner->io()->writeln(
                '[tenant:' . $item->id . '] provision выполнен.',
                'success',
            );
        }
